...
from analysis import movie, movie2d, plot_wf
...
sequence_index = int(get_var('sequence_index', h5tools.autosequence('run') - 1))
...
def task_movie():
    for filename in filenames:
        ...
        yield {
            'name': run_number,
            'actions': [(movie2d.make_movie, [filename, 20, None], {})],
        }


def task_movie_slice():
    for filename in filenames:
        run_number = int(filename.stem[-4:])
        yield {
            'name': run_number,
            'actions': [(movie.make_movie1d, [filename, 20, None], {})],
        }
...
def task_plot():
    for filename in filenames:
        ...
        yield {
            'name': run_number,
            'actions': [(plot_wf.plot_wavefunction, [filename, 'groundstate'], {})],
        }
